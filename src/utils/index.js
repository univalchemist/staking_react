...
export function formatNumberWithCommas(x, decimal = 4) {
  const valToFixed = Number(x).toFixed(decimal).toString();
  const intPart = valToFixed.split('.')[0];
  const decimalPart = valToFixed.split('.')[1];
