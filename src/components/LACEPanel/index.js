...
import { NETWORK_CHAIN_ID } from 'constants/variables';
...
const LACEPanel = ({
  ...
  const [lastSuccessAction, setLastSuccessAction] = useState('');
  ...
  const approve = async () => {
    ...
    setResetInput(false);
    setCurrentLoadingProgress('INCREASING');
    ...
    if (increaseAloowanceResult) {
      ...
      setLastSuccessAction('INCREASING');
    }
  ...
  const unstake = async () => {
    setIsSuccessAction(false);
    setResetInput(false);
    setCurrentLoadingProgress(data.limit ? 'COLLECTING' : 'UNSTAKING');
    ...
    if (stakingResult) {
      setIsSuccessAction(true);
      setResetInput(true);
      setLastSuccessAction(data.limit ? 'COLLECTING' : 'UNSTAKING');
    }
  ...
  const collect = async () => {
    if (data.limit) { // If Guaranteed APY Staking, it should call unstake function.
      unstake();
      return;
    }
  ...
  useEffect(() => {
    if (currentAddress && chainId === NETWORK_CHAIN_ID) {
      defaultData();
    } else {
      setCurrentData(null);
    }
