import { NETWORK_PARAMS } from 'constants/blockchain-networks-params';
import { BLOCKCHAIN_WORKSPACES, NETWORK_CHAIN_ID } from 'constants/variables';
...
class AuthStore {
  ...
  @action.bound setChainIdValue(value) {
    if (value === NETWORK_CHAIN_ID) {
      this.chainId = value || 0;
      this.network = AuthHelper.getNetworkByChainId(value);
    } else {
      const network = NETWORK_PARAMS[BLOCKCHAIN_WORKSPACES.BINANCE];
